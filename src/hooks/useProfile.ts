  ...
  return useQuery<{ stats: ProfileStats; badges: BadgeRow[]; monthCount: number }>({
    ...
    queryFn: async () => {
      ...
      const [memRes, badgesRes, monthRes] = await Promise.all([
        supabase.from('membership').select('points, visits, streak, level').eq('forening_id', fid).eq('user_id', uid).maybeSingle(),
        ...
        supabase.from('checkin').select('id', { count: 'exact', head: true }).eq('forening_id', fid).eq('user_id', uid).gte('created_at', startOfMonth.toISOString()),
      ]);
